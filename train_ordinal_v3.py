...
v3 相对 v2：按训练集类别频率为各 CORAL 阈值计算 BCE pos_weight，缓解阈值正负样本不平衡。
"""
...
def compute_coral_pos_weight(
    class_counts: List[int], num_classes: int
) -> torch.Tensor:
    """各 CORAL 阈值 j 的 BCE pos_weight = neg/pos；target=1 当且仅当 label > j。"""
    num_coral = num_classes - 1
    weights = []
    for j in range(num_coral):
        pos = float(sum(class_counts[j + 1 :]))
        neg = float(sum(class_counts[: j + 1]))
...
def compute_ordinal_loss(
    ...
    loss = coral_loss(logits, coral_targets, pos_weight=pos_weight)
    if emd_weight > 0.0:
    ...
    for images, labels in loader:
        ...
        with torch.amp.autocast(device_type=device.type, enabled=use_amp):
            ...
            loss = compute_ordinal_loss(
                logits, targets, class_probs, emd_weight, pos_weight=pos_weight
            )
...
def main():
    ...
    pos_weight = compute_coral_pos_weight(class_counts, num_classes).to(device)
    print(
        f"[train] head=CORAL (K-1={num_coral} logits) | 训练集类别计数 {class_counts} | "
        f"CORAL pos_weight={[round(w, 3) for w in pos_weight.tolist()]} | "
